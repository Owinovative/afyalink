...
final readonly class SubmissionReadinessChecker
{
    ...
    public function evaluate(
        ...
        if ($track === ApplicantTrack::StudentAwaitingLicense->value) {
            foreach (['name', 'email', 'phone', 'target_profession', 'student_status', 'institution_name', 'programme_or_course', 'county'] as $field) {
                if (!isset($profile[$field]) || trim((string) $profile[$field]) === '') {
                    $missing[] = "profile.$field";
                }
            }

            foreach ($this->requirements->minimumRequiredDocumentsForTrack($track, (string) ($profile['target_profession'] ?? $profile['profession'] ?? '')) as $documentType) {
            ...
            $missing[] = 'track.licensed_professional_conversion_required';
        ...
        foreach ($this->requirements->minimumRequiredDocumentsForTrack($track, (string) ($profile['profession'] ?? '')) as $documentType) {
            $status = $credentialStatuses[$documentType->value] ?? null;
            $statusValue = $status instanceof CredentialReviewStatus ? $status->value : (string) $status;
